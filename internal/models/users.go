...
func (a *User) Create(db DB) *errs.Error {
	...
	if exists {
		return errs.New().SetCode(errs.ERROR_ALREADY_EXISTS).SetMsg("user with login %s already exists", a.Login)
	}
...
func (a *User) LogIn(db DB) *errs.Error {
	...
	if !exists {
		return errs.New().SetCode(errs.ERROR_NOT_FOUND).SetMsg("user %s not found", a.Login)
	}
...
func LoadUserByLogin(db DB, login string) (*User, *errs.Error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, errs.New().SetCode(errs.ERROR_NOT_FOUND).SetMsg("%s", err)
		}
		return nil, errs.New().SetCode(errs.ERROR_INTERNAL).SetMsg("%s", err)
	}
	return u, nil
